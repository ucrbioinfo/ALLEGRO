...
namespace solvers {
    ...
    std::string ORToolsSolver::decode_bitset(boost::dynamic_bitset<> &encoded)
    {
        std::string decoded = "";
        std::string buffer;
    ...
    std::string ORToolsSolver::decode_bitset(const std::string &encoded_str)
    {
    ...
    std::unordered_set<std::string> ORToolsSolver::randomized_rounding(
    ...
    std::unordered_set<std::string> ORToolsSolver::ortools_solver()
    {
        ...
        for (auto i : *(this->coversets))
        {
            ...
            if (
                (species_bitset.count() == 1) &&
                (species_already_hit_by_unique_guide.find(species_bitset) != species_already_hit_by_unique_guide.end()))
            {
