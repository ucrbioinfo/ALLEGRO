...
#include <pybind11/stl.h> // Allows automatic conversion of std::vector, std::map, etc.
...
namespace Kirschtorte
{
        ...
        std::size_t container_id)
    {
        ...
        for (auto base : seq)
        {
            ...
            {
            case 'A':
                encoded_str += sA_SHIFT;  // "00" – definitions.h
                break;
            case 'C':
                encoded_str += sC_SHIFT;  // "01" – definitions.h
                break;
            case 'G':
                encoded_str += sG_SHIFT;  // "10" – definitions.h
                break;
            case 'T':
                encoded_str += sT_SHIFT;  // "11" – definitions.h
                break;
            default:
    ...
    std::vector<GuideStruct> Kirschtorte::setup_and_solve()
    {
        ...
        LOG(INFO) << "Number of variables: " << this->solver->NumVariables();
        this->log_buffer << "Number of variables: " << this->solver->NumVariables() << std::endl;
        // --------------------------------------------------
        ...
        if (len_solutions > 0)
        {
            ...
            {
                std::cout << BLUE << "> " << RESET << "Number of feasible candidate targets: " << len_solutions << "." << std::endl;
                std::cout << BLUE << "> " << RESET << "Switching to the ILP solver as " << num_fractional_vars << " residual targets remain." << std::endl;
                this->log_buffer << "Number of feasible candidate targets: " << len_solutions << "." << std::endl;
            ...
            {
                std::cout << BLUE << "> " << RESET <<  "Your library includes " << len_solutions << " targets." << std::endl;
                ...
                for (auto var_ptr : feasible_solutions)
                {
                    ...
                    GuideStruct guide;
                    guide.sequence = decoded_bitset;  // This will be in binary and NOT a string like "ACTGTG..."
                    guide.score = score;  // double
                    guide.species_hit = buffer;  // string – looks something like this "010101001"
...
PYBIND11_MODULE(_kirschtorte_core, m) {
    py::class_<GuideStruct>(m, "GuideStruct")
        .def(py::init<>())
        .def_readwrite("sequence", &GuideStruct::sequence)
        .def_readwrite("score", &GuideStruct::score)
        .def_readwrite("species_hit", &GuideStruct::species_hit);

    // We name the C++ class "_KirschtorteEngine" internally
    py::class_<Kirschtorte::Kirschtorte>(m, "_KirschtorteEngine")
        .def(py::init<
            bool, 
            std::size_t, 
            std::size_t, 
            ...
            std::string
        >())
        .def("encode_and_save_dna", &Kirschtorte::Kirschtorte::encode_and_save_dna)
        .def("setup_and_solve", &Kirschtorte::Kirschtorte::setup_and_solve);
}
