...
class ScorerFactory:
    ...
    def get_scorer(self, scorer_name: str, scorer_settings: dict[str, str]) -> Scorer:
        ...
        match scorer_name:
            ...
            # case 'deepguide_1':
            #     return DeepGuideWrapper(scorer_settings)
            case 'random':
                return RandomScorer()  # Assigns all guides a random score between [0, 10)
            case _:
                print('Unknown scorer selected. Assigning all guides a default score of 1.0')
                return DummyScorer()