...
import coverset2  # Cythonized custom C++ lib, AKA coverset.so
...
cdef extern from "allegro/coverset2.h" namespace "coversets":
...
cdef class CoversetsCython:
    ...
    def __cinit__(
        ...
        self.num_genes = 8  # fix todo
        self.num_species = species_df.shape[0]
        self.num_containers = self.num_species * self.num_genes

        self.coverset = new CoversetCPP(self.num_containers, guide_length, num_trials, output_directory.encode('utf-8'))
        ...
        for row in species_df.itertuples():
            ...
            for guide_container in guide_containers_list:
                ...
                for guide_object in guide_objects_list:
                    ...
                    status = self.coverset.encode_and_save_dna(
        ...
        print()
        print('Created coversets for all genes.')
        ...
        winners_bytes_pairs = self.coverset.ortools_solver(gene_cut_multiplicity, mp_threshold, beta)
