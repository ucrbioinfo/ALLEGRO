#include "allegro/rammingstone.h"

namespace Rammingstone
{
    Rammingstone::Rammingstone(
    ...
    int Rammingstone::encode_and_save_dna(
        std::string seq,
        std::size_t score,
    ...
    std::vector<std::pair<std::string, std::string>> Rammingstone::setup_and_solve(
        std::size_t monophonic_threshold,
        std::size_t cut_multiplicity,
        std::size_t beta)
    {
        std::unordered_map<boost::dynamic_bitset<>, std::unordered_set<boost::dynamic_bitset<>>> hit_containers;
        ...
        for (auto i : hit_containers)
        {
            ...
            // All species must be covered by at least cut_multiplicity guide(s)
        ...
        if (beta > 0)
        {
            LOG(INFO) << "Number of constraints (genes) + 1 (beta): " << solver->NumConstraints();
            this->log_buffer << "Number of constraints (genes) + 1 (beta): " << solver->NumConstraints() << std::endl;
        }
        else
        {
